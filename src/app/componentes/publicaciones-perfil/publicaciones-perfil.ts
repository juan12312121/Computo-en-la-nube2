...
export class PublicacionesPerfil implements OnChanges, OnInit, OnDestroy {
  ...
  constructor(
    ...
    console.log('🔧 PublicacionesPerfil usando BASE_URL:', this.BASE_URL);
  }
  ...
  private actualizarPostsDesdeAPI(): void {
    this.posts = this.publicaciones.map(pub => {
      ...
      const post: Post = {
        ...
        showComments: false,
        totalComments: pub.total_comentarios || 0,
        ...
        next: (response) => {
          if (response.success && response.data) {
            ...
            if (response.data.foto_perfil_url) {
              fotoPerfilUrl = this.construirUrlLocal(response.data.foto_perfil_url);
            }
        ...
        next: (response) => {
          if (response.success && response.data) {
            response.data.forEach(usuario => {
              const urlFinal = this.construirUrlLocal(usuario.foto_perfil_url);
  ...
  // ✅ NUEVA FUNCIÓN: Construir URL local
  private construirUrlLocal(ruta: string | null | undefined): string | null {
    if (!ruta) return null;
    ...
    if (ruta.startsWith('/uploads/')) {
      return `${this.BASE_URL}${ruta}`;
    }

    // Si no tiene prefijo, asumir que es relativa
    if (!ruta.startsWith('http')) {
      return `${this.BASE_URL}${ruta.startsWith('/') ? ruta : '/' + ruta}`;
    }
  ...
  private obtenerUrlImagen(pub: Publicacion): string | null {
    // ✅ Priorizar imagen_url (local)
    if (pub.imagen_url) {
      return this.construirUrlLocal(pub.imagen_url);
    }

    // ✅ Fallback a imagen_s3 (por si acaso)
    if (pub.imagen_s3) {
      return this.construirUrlLocal(pub.imagen_s3);
    }
  ...
  private obtenerUrlFotoPerfil(pub: Publicacion): string | null {
    ...
    if (fotoPerfil) {
      return this.construirUrlLocal(fotoPerfil);
    }

    return this.usuarioFotoPerfil ? this.construirUrlLocal(this.usuarioFotoPerfil) : null;
  }
