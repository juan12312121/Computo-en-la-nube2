...
export class DetallePost implements OnInit, OnDestroy {
  ...
  private setupSocketListeners(): void {
    ...
    this.socketService.onEvent<{ publicacion_id: number, total: number }>('like_update')
      .subscribe(data => {
        if (this.post && this.post.id === data.publicacion_id) {
          this.post.likes = data.total;
          this.cdr.detectChanges();
        }
      });

    // Escuchar nuevos comentarios
    this.socketService.onEvent<any>('new_comment')
      .subscribe(data => {
        if (this.post && this.post.id === data.publicacion_id) {
          // Evitar duplicados (si el usuario actual fue quien comentó)
          if (!this.comentariosAPI.some(c => c.id === data.id)) {
            // Transformar el comentario recibido al formato de la UI
            const newComment: Comment = {
              id: data.id,
              author: data.nombre_completo || data.nombre_usuario || 'Usuario',
              avatar: this.generarAvatarIniciales(data.nombre_completo || data.nombre_usuario || 'U'),
              text: data.texto,
              time: 'Ahora',
              avatarColor: this.generarColorAvatar(data.usuario_id),
              usuario_id: data.usuario_id,
              foto_perfil_url: data.foto_perfil_url || null,
              usandoIniciales: !data.foto_perfil_url,
              canDelete: data.usuario_id === this.usuarioActualId
            };

            this.comentariosFormateados.unshift(newComment);
            this.comentariosAPI.push(data);
            this.cdr.detectChanges();
          }
        }
      });
